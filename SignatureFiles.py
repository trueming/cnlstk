...
class SignatureFiles:
	...
	def __init__(self, folder):
		...
		self.__getFiles__(folder)
		self.Filelist.sort()

	def __getFiles__(self, folder):
		...
		for k in Li:
			if os.path.isdir(folder+k):
				self.__getFiles__(folder+k+'/')
			else:
				self.Filelist.append(folder+k)

	def __c_mul__(self, a, b):
		return eval(hex((long(a) * b) & 0xFFFFFFFFL)[:-1])

	def __uni2ord__(self, uni, ot):
		...
		if ot == "two":
			...
			cc = ord(uni[0])<<7
			for char in uni:
				cc = self.__c_mul__(1000003, cc) ^ ord(char)
			cc = cc ^ len(uni)
			if cc == -1:
				cc = -2
	...
	def getSFiles(self, uni_folder, bi_folder):
				...
				for k in line:
					if checkChineseUnicode(k)=="yes":
						if k in tmp_uni:
							...
							tmp_uni.append(k)
						gcc = self.__uni2ord__(k, "one")	#取得該字的餘數
#						print k, gcc
#						raw_input()
				...
				for k in line:
					if checkChineseUnicode(k)=="yes":	#只處理中文
						...
						if bgw in tmp_bi:	#重複的雙字詞跳過不處理
							...
							tmp_bi.append(bgw)
						gcc = self.__uni2ord__(bgw, "two")	#取得雙字詞的餘數
#						print bgw, gcc
#						raw_input()
...
#			print "bi-gram ok!"
		...
		print "outputing ..."
		rst = self.__outputSFiles__(uni_folder, self.No_uni, self.Dic_uni)
		rst = self.__outputSFiles__(bi_folder, self.No_bi, self.Dic_bi)
		
		return ( len(self.Dic_uni), len(self.Dic_bi), len(self.Word_combine) )
		
	def __outputSFiles__(self, folder, no, dic):
	...
	def doSearch(self, term, SF_u, SF_b):
		u = []
		b = []
		tmp_k = ""
		for k in term:
			u.append([k, self.__uni2ord__(k, "one")])
			if tmp_k != "":
				tmp_k += k
				b.append([tmp_k, self.__uni2ord__(tmp_k, "two")])
			tmp_k = k
...
#				self.__countSF__(k[0], SF_u + str(k[1]))
		for k in b:
			if os.path.exists( SF_b + str(k[1]) ) == True:
				self.__countSF__(k[0], SF_b + str(k[1]))
...
#				raw_input()
				f = codecs.open(self.Filelist[k], 'r', 'utf8')
				l = f.read()
				f.close()
				if kw in l:
...
if __name__ == "__main__":
	...
	source_path = "/media/Index/Cbeta_Source"
	SignatureFiles_path_u = "/media/Index/Cbeta_SignatureFiles/u/"
	SignatureFiles_path_b = "/media/Index/Cbeta_SignatureFiles/b/"
	
	obj = SignatureFiles(source_path)

	print len(obj.Filelist)
#	for k in obj.Filelist:
#		print k

#	rst = obj.getSFiles( "/media/Index/Cbeta_SignatureFiles/u", "/media/Index/Cbeta_SignatureFiles/b" )
#	print "No. of uni_gram: %d, bi_gram: %d, combine_word: %d" % (rst[0], rst[1], rst[2])
		
#	obj.checkSFiles(u"研;二義;鳥;飛行人;十一", SignatureFiles_path_u, SignatureFiles_path_b)
	
	rst = obj.doSearch(u"二義非行人十一", SignatureFiles_path_u, SignatureFiles_path_b)
#	for k in rst:
#		k
